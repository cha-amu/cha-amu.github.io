...
실제 Sheets까지 쓰는 로컬 sync는 `API_URL`과 `STORAGE_SYNC_SECRET`이 필요하므로, 보통은 GitHub Actions 수동 실행을 쓴다. 관리자 비밀번호나 관리자 세션은 사용하지 않는다. `API_URL`은 Apps Script 원본이 아니라 `https://cha-amu-gateway.yiyaaang.workers.dev/api`를 사용한다.
