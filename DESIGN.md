...
- Primary navigation: 채널명/로고는 사이트 상단 전역줄 왼쪽에 둔다. `아무 글`, `자료`는 그 아래 줄의 큰 정사각형 메뉴로 두고, `방명록`, `검색`, `설정`은 사이트 상단 오른쪽 공통 도구로 둔다.
...
## Design principles
- Principle 1: 같은 위계는 같은 크기와 위치 규칙을 쓴다. 특히 채널명과 공통 도구는 모든 페이지에서 전체 뷰포트 기준 fixed 상단 레일을 공유하고, 주요 탭은 그 아래 별도 행으로 읽히게 한다.
